...
function SearchableList(props) {
  ...
  const actionButton = btnText ? (
    <button className="btn btn-primary float-right" onClick={btnAction}>
      {btnText}
    </button>
  ) : null;
  ...
  return (
    <div>
      ...
      <ul className="list list-group search-list-results overflow-auto">
        {listItems}
      </ul>
    </div>
  );

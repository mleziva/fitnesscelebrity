...
export class FitnessPathService {
    ...
    async getSubscribedPaths(userId, params) {
        const url = FitnessPathRoutes.GetSubscribedPaths.replace("{userid}",userId) + Helper.buildQueryString(params);
        const token = await authService.getAccessToken();
        const response = await fetch(url, {
            method: 'GET',
            headers: { 
    ...
    async getSubscribers(fitnessPathId) {
        const url = FitnessPathRoutes.GetSubscribers.replace("{fitnesspathid}",fitnessPathId)
        const token = await authService.getAccessToken();
        const response = await fetch(url, {
            method: 'GET',
            headers: { 
